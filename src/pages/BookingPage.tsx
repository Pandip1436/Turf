...
function calcTotal(slots: SlotInfo[], selected: Set<string>): number {
  ...
  if (count >= 3)  return Math.round(base * 0.8);
  return base;
}
...
const BookingPage = () => {
  ...
  // Scroll to top on step/confirm change
  ...
  const slotClass = (slot: SlotInfo) => {
    if (!slot.available && !slot.isYours)
      return 'bg-gray-100 border border-gray-200 text-gray-400 cursor-not-allowed opacity-60';
    if (slot.isYours)
      return 'bg-blue-100 border-2 border-blue-400 text-blue-700 cursor-default';
    if (selectedSlots.has(slot.slot))
      return 'bg-green-600 border-2 border-green-700 text-white cursor-pointer';
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setBookingRef(res.data.booking?.bookingRef || `HG${Date.now().toString().slice(-6)}`);
      setConfirmed(true);
    } catch (err: unknown) {
      ...
      setBookingError(msg);
      // If slots conflict, refresh slot availability
      if (e.response?.data?.takenSlots) fetchSlots(date);
    } finally {
      setLoading(false);
    }
  };
          ...
          {stepLabels.map((lbl, idx) => {
            ...
            return (
              <React.Fragment key={s}>
                ...
                {idx < 2 && <div className={`h-0.5 w-8 sm:w-16 mx-1 transition-colors ${done ? 'bg-green-500' : 'bg-gray-200'}`} />}
              </React.Fragment>
            );
          })}
        </div>
          ...
          <div className="bg-white w-full rounded-2xl shadow-sm p-6 sm:p-8">
            ...
            <div className="bg-gray-50 rounded-xl p-5 mb-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-500">Slots</span>
                  <span className="font-semibold">{selectedSlots.size} slot{selectedSlots.size !== 1 ? 's' : ''}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Name</span>
                  <span className="font-semibold">{form.name}</span></div>
                {form.teamSize && <div className="flex justify-between"><span className="text-gray-500">Team Size</span>
                  <span className="font-semibold">{form.teamSize} players</span></div>}
              </div>
            </div>
            ...
            {bookingError && (
              <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-4 flex items-center gap-2">
                <span className="text-base">⚠️</span> {bookingError}
              </div>
            )}
            ...
            <p className="text-center text-xs text-gray-400 mb-5 flex items-center justify-center gap-3">
              <span>🔒 Secure Payment</span><span>•</span><span>UPI, Cards, Netbanking</span>
            </p>
